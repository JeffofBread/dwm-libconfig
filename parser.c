...
static int _parse_buttonbind( const char *buttonbind_string, const unsigned int max_keys, Button *parsed_buttonbind ) {
        ...
        if ( _parse_bind_function( function_token, &parsed_buttonbind->func, &parsed_buttonbind->argument_type, &range_min, &range_max ) ) {
...
static int _parse_generic_settings( const config_t *libconfig_config, unsigned int *max_keys ) {
        ...
        for ( int i = 0; i < LENGTH( setting_map ); ++i ) {
                switch ( setting_map[ i ].type ) {
                        case TYPE_BOOL:
                                settings_failed_count -= libconfig_lookup_bool( libconfig_config, setting_map[ i ].name, setting_map[ i ].optional, setting_map[ i ].value );
                                break;
                        case TYPE_INT:
                                settings_failed_count -= libconfig_lookup_int( libconfig_config, setting_map[ i ].name, setting_map[ i ].optional, (int) setting_map[ i ].range_min,
                                                                               (int) setting_map[ i ].range_max, setting_map[ i ].value );
                                break;
                        case TYPE_UINT:
                                settings_failed_count -= libconfig_lookup_uint( libconfig_config, setting_map[ i ].name, setting_map[ i ].optional, (unsigned int) setting_map[ i ].range_min,
                                                                                (unsigned int) setting_map[ i ].range_max, setting_map[ i ].value );
                                break;
                        case TYPE_FLOAT:
                                settings_failed_count -= libconfig_lookup_float( libconfig_config, setting_map[ i ].name, setting_map[ i ].optional, (float) setting_map[ i ].range_min,
                                                                                 (float) setting_map[ i ].range_max, setting_map[ i ].value );
                                break;
                        case TYPE_STRING:
                                settings_failed_count -= libconfig_lookup_string( libconfig_config, setting_map[ i ].name, setting_map[ i ].optional, setting_map[ i ].value );
                                break;
                }
        }
...
static int _parse_keybind( const char *keybind_string, const unsigned int max_keys, Key *parsed_keybind ) {
        ...
        long double range_min, range_max;
        if ( _parse_bind_function( function_token, &parsed_keybind->func, &parsed_keybind->argument_type, &range_min, &range_max ) ) {
...
static int _parse_rule( const config_setting_t *rule_libconfig_setting, const int rule_index, Rule *parsed_rule ) {
        ...
        failed_rule_elements_count -= libconfig_setting_lookup_uint( rule_libconfig_setting, "tag-mask", false, 0, TAGMASK, &parsed_rule->tags );
        failed_rule_elements_count -= libconfig_setting_lookup_int( rule_libconfig_setting, "monitor", false, -1, 99, &parsed_rule->monitor );
        failed_rule_elements_count -= libconfig_setting_lookup_int( rule_libconfig_setting, "floating", false, 0, 1, &parsed_rule->isfloating );
...
static int _parse_rule_string( const config_setting_t *rule_libconfig_setting, const char *path, const int rule_index, char **parsed_value ) {
        ...
        libconfig_setting_lookup_string( rule_libconfig_setting, path, false, &tmp_string );
        if ( tmp_string == NULL ) {
...
static int _parse_theme( const config_setting_t *theme_libconfig_setting ) {
        ...
        for ( int i = 0; i < LENGTH( theme_map ); i++ ) {
                if ( !libconfig_setting_lookup_string( theme_libconfig_setting, theme_map[ i ].path, false, &tmp_string ) ) {
 ...
 *
 * @note TODO mention dynamic allocation with _join_strings() and estrdup()
 *
 * @note This function is derived from [picom's](https://github.com/yshui/picom/) xdg_config_home().
 * Credit more or less goes to [Yuxuan Shui](yshuiv7@gmail.com), I just made some minor adjustments.
 *
